...
- [x] 5.5c Reopen orchestrator — shipped in `dispatchPersistentTurn`: on `hashMismatch`, calls `manager.drop()` to close the old runtime, then `createRuntime` with the existing `claudeSessionId` or the cache-provided id as `resumeSessionId`, then re-enters the turn on the new runtime without losing the pushed user message. 1 test (`reopens via close+cold-reattach when a reopen-critical option changes`) confirms the carry-forward.
