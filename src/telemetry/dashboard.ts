...
function render(s, reqs, logs) {
  ...
  for (const r of reqs) {
    ...
    html += '<tr>'
      ...
      + '<td>' + r.mode + (r.hasDeferredTools ? '<br><span style="font-size:10px;color:var(--purple)">always=' + ((r.toolCount || 0) - (r.deferredToolCount || 0)) + ' deferred=' + (r.deferredToolCount || 0) + '</span>' + (r.discoveredTools && r.discoveredTools.length > 0 ? '<br><span style="font-size:10px;color:var(--green)">+' + r.discoveredTools.join(', +') + '</span>' : '') : '') + '</td>'
